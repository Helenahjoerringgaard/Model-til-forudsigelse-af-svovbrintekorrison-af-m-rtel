...
flow_g_mellem = (2731.5.*variation_byspildevand_g)/10; % sommer [m3/s]
%flow_g_mellem_best = (6619.3.*variation_byspildevand_g)/10; % sommer [m3/s]
%flow_g_mellem_worst = (2010.2.*variation_byspildevand_g)/10; % sommer [m3/s]
...
% Der laves matricer
areal_gasfase_delfuld_g_matrix = areal_gasfase_delfuld_g.*ones(8640,14);
volume_gasfase_delfuld_g_matrix = volume_gasfase_delfuld_g.*ones(8640,14);
...
KLa_h2s_matrix_g = [KLa_h2s_g.*2.*ones(8640,2) KLa_h2s_g.*ones(8640,12)]; %[1/s]

% Fraktion af H2S (aq) i vandfasen; pKa = 7.1
H2S_aq = C_total./(exp(-log(10)*7.1 + log(10)*pH)+1); %[g H2S/m3]
H2S_vandfase = [H2S_aq.*ones(8640,2) zeros(8640,12)]; 
...
% Henrys konstant korrigeret efter temperatur
henrys_konstant = 1./ (0.1.*exp( 2100*(1./(temperatur+273.15) - 1./298.15) ) ) .*ones(8640,14); %[atm]

% Kinetiske parametre
K_F = 15.52*ones(8640,14);
n = 0.55*ones(8640,14);
...
H2S_gasfase = [ 0.75.*koncentration_H2S_broend.*ones(8640,2) zeros(8640,12) ] ;
H2S_gasfase_ppm = [ 0.75.*(koncentration_H2S_broend./konversionsfaktor).*ones(8640,2) zeros(8640,12) ];
...
H2S_oxidation_gm2s = [( 15.52.*(0.75.*(koncentration_H2S_broend./konversionsfaktor)).^0.55).*0.001*(1/3600).*ones(8640,2) zeros(8640,12)]; %[g S/m2*s]
H2S_oxidation = zeros(8640,14);

% Simulering af frigivelse af H2S og adsorption samt oxidation p? betonr?rets overflade ned gennem den forede ledning
for i=2:8640
    for j=2:14 % F?rste kolonne er en fiktiv nedre rand
        ...
        if (H2S_frigivelse(i,j) + 0.7896*H2S_gasfase(i-1,j) + 0.1052.*H2S_gasfase(i-1,j-1) ) > H2S_oxidation(i-1,j) %+ 0.1.*H2S_gasfase(i-1,j-1)
            
            H2S_gasfase(i,j) = H2S_frigivelse(i,j)  + 0.7896.*H2S_gasfase(i-1,j) + 0.1052.*H2S_gasfase(i-1,j-1) -  H2S_oxidation(i-1,j) ; %[g S/m3] + H2S_gasfase(i,j=2)
...
H2S_roerprofil_oxidation_sommer = H2S_oxidation(:, 2:end);
H2S_roerprofil_vandfase_sommer = H2S_vandfase(:, 2:end);
H2S_roerprofil_gas_sommer = H2S_gasfase(:, 2:end);
H2S_roerprofil_gas_ppm_sommer = H2S_gasfase_ppm(:, 2:end);
...
korrosionsrate_sommer = 11.4.*k_korrosion.*((H2S_oxidation_gm2s.*3600)./A); %[mm/?r]
korrosionsrate_sommer_mean = mean(korrosionsrate_sommer(:,2:end))
%korrosionsrate_sommer_mean_best = mean(korrosionsrate_sommer(:,2:end));
%korrosionsrate_sommer_mean_worst = mean(korrosionsrate_sommer(:,2:end));
...
%nedstroems_ppm_sommer_best = H2S_roerprofil_gas_ppm_sommer(:,13);
%opstroems_ppm_sommer_best = H2S_roerprofil_gas_ppm_sommer(:,1);

%nedstroems_ppm_sommer_worst = H2S_roerprofil_gas_ppm_sommer(:,13);
%opstroems_ppm_sommer_worst = H2S_roerprofil_gas_ppm_sommer(:,1);
%% PLOTS
...
% figure(5)
% surf((1:13), (1:8640), H2S_roerprofil_gas_ppm_sommer, 'FaceAlpha','0.9', 'EdgeColor','none')
...
% figure(6)
%nedstroems_ppm = H2S_roerprofil_gas_ppm_sommer(:,10);
%opstroems_ppm = H2S_roerprofil_gas_ppm_sommer(:,1);
