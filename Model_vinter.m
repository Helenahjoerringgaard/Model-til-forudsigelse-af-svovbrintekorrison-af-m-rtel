...
% Tidspunkter hvor pumpen, der leder vand til trykledningen, koerer
pumpeaktiv_mellem = OpholdstiderS3(:,6); %[-]

% Opholdstider til naer pumpen, der leder vand til trykledningen, koerer
opholdstid_mellem = OpholdstiderS3(:,3)*(60*60*24).*pumpeaktiv_mellem; % [s]

% Vandfoering i loebet af dagen i den forede ledning
variation_byspildevand_g = OpholdstiderS3(:,8);
flow_g_mellem = (6281.5.*variation_byspildevand_g)/10; % vinter [m3/s]

% Hvor meget vand der udledes naer pumpen koerer
...
%stof er mellem 0,007-0,010 g S gO2 / m*h
%COD antages 950 g O2/m3
temperatur = 8; % grader
...
% ovenfor naevnte rate, areal/volumenforholdet og opholdstider [gS/m3]
...
Ruhed_PVC = 0.00003; %[m] Denne er antaget fra "normal" PVC roer = 0,01-0,05 mm 
Manningtal_PVC = 25.4/(Ruhed_PVC)^(1/6); %[-]

% Vandfoering naer PVC-gravitationsledningen er fuldtloebende
...
Areal_fuld_PVC = pi*((Diameter_PVC/2)^2); %[m2] radius for fuldloebende roer = diameter/4
Q_fuld_PVC = v_fuld_PVC*Areal_fuld_PVC; %[m3/s]

% Vanddybde afhaengig af vandfoering (Q), naer PVC-gravitationsledningen er
% delfyldt
...
% Froudes tal bestemmer om stroemningen er strygende eller stroemmende, 
% <1: stroemmende og >1: strygende
Froude_PVC = v_delfuld_PVC.*(9.816.*middel_vanddybde_PVC).^(-0.5); % [-]

% Geniltningskoefficient gaeldende for PVC-gravitationsledningen 
...
% Maksimale maengde ilt, som kan optages i vandet i PVC-gravitationsledningen
ilt_max_optag_PVC = 14.652 - 0.41022*temperatur+0.00799*temperatur^2-0.0000777*temperatur^3; % [g/m3]; og [C] - ER ENHEDEN IKKE KUN G/m3?

% Geniltningsrate i PVC-gravitationsledningen
geniltningsrate = KLa_ilt_PVC*(ilt_max_optag_PVC-0); % [g/m3*s] - 0 er iltkoncentration til start
total_ilt_optag_i_PVC = (Laengde_PVC./v_delfuld_PVC).*geniltningsrate;% [g O2/m3]
...
% Maengde sulfid der kommer ud af PVC-gravitationsledningen: 
% maengde sulfid, der oxideres, traekkes fra den totale maengde sulfid, 
...
% Vaerdien for pH er antaget
pH = 6.9; % [-]

% Svovlbrinteudvekslingskoefficient beregnes paa baggrund af
% iltudvekslingskoefficienten og en omregningsfaktor
KLa_h2s_PVC = max(KLa_ilt_PVC)*0.84; % [1/s]

% Der oprettes matricer for at forkorte beregningstiden i loopet
KLa_h2s_matrix_PVC= KLa_h2s_PVC.*ones(8640,263); %[1/s]

% Fraktion af H2S (aq) i vandfasen; pKa = 7.1
H2S_aq_PVC = C_PVC./(exp(-log(10)*7.1 + log(10)*pH)+1); %[g S/m3]
H2S_vandfase_PVC = [H2S_aq_PVC.*ones(8640,2) zeros(8640,261)]; %

% Konversionsfaktor af ppm til g/m3 og omvendt
volume_idealgas_temperatur = (1*8.314472*(273.15+temperatur))/101325; % KAN DENNE BARE INKORPERERES I LIGNINGEN NEDENUNDER?
konversionsfaktor = (1/volume_idealgas_temperatur)*32.065*10^-6;

% Henrys konstant korrigeret efter temperatur
henrys_konstant = 1./ (0.1.*exp( 2100*(1./(temperatur+273.15) - 1./298.15) ) ) .*ones(8640,263); %[atm] - MENER DET ER L M3 M2 ELLER NOGET - IKKE ATM I HVERT FALD

% Frigivelse af H2S fra vand- til gasfase; der ganges med tidsskridtet paa
...
Diameter_g = 0.90-(2*0.01); % [m] Medregnet tykkelse af moertel
Ruhed_g = 0.003; %[m] Denne er antaget fra "normal" betonroer
Manningtal_g = 25.4/(Ruhed_g)^(1/6); %[-]

% Vandfoering i den forede ledning, naar den er fuldtl?bende
...
% Vanddybde afhaengig af vandfoering (Q)
...
% Nedenstaeende beregninger leder hen til beregningerne af volumen af vand-
% og gasfase

% Teta_g er vinklen mellem centrum af roeret til hvor vandspejlet rammer
% betonroerets overflade
teta_g = acos(1-((2.*vanddybde_g)/Diameter_g));% [rad]

% Tvaersnitsarealet af vandfasen
areal_vand_g = Diameter_g^2/8*(2*teta_g-sin(2*teta_g));%[m2]

% Hastighed af vandet i delfyldt roer
v_delfuld_g = flow_g_mellem./areal_vand_g; %[m/s]

% Middelvanddybde i delfyldt roer
...
% Fraktion af H2S (aq) i vandfasen; pKa = 7.1
H2S_aq = C_total./(exp(-log(10)*7.1 + log(10)*pH)+1); %[g S/m3]
...
% Henrys konstant korrigeret efter temperatur
henrys_konstant = 1./ (0.1.*exp( 2100*(1./(temperatur+273.15) - 1./298.15) ) ) .*ones(8640,11); %[atm] - SAMME HER

% Kinetiske parametre
K_F = 15.52*ones(8640,11);
n = 0.55*ones(8640,11);

% Frigivelse af H2S fra vand- til gasfase; der ganges med tidsskridtet paa 10 sekunder
% Gennemsnitshastigheder der afgoer antal stedsskridt (kolonner), j: 
% vandf?ring forede ledning er 0,62 m/s = 10 stedsskridt + nedre rand
% Gasfasens hastighed er derfor 1/12,4 = 0,0809 af vandfasen


H2S_gasfase = [ 0.75.*koncentration_H2S_broend.*ones(8640,2) zeros(8640,9) ] ; %ganges med 0,75 fordi 25% forsvinder pga. dispersion
H2S_gasfase_ppm = [ 0.75.*(koncentration_H2S_broend./konversionsfaktor).*ones(8640,2) zeros(8640,9) ]; %- enheder?
...
for i=2:8640
    for j=2:11 % Foerste kolonne er en fiktiv nedre rand
        ...
        if (H2S_frigivelse(i,j) + 0.8381*H2S_gasfase(i-1,j) + 0.0809.*H2S_gasfase(i-1,j-1) ) > H2S_oxidation(i-1,j) %
            
            H2S_gasfase(i,j) = H2S_frigivelse(i,j)  + 0.8381.*H2S_gasfase(i-1,j) + 0.0809.*H2S_gasfase(i-1,j-1) -  H2S_oxidation(i-1,j) ; %[g S/m3] 
...
H2S_roerprofil_oxidation_vinter_gm2s = H2S_oxidation_gm2s(:, 2:end);
...
% Alkalinitet af den paafoerte moertel; antagelse: omkring 0.6-0.8, men normalt er den 0,2
A = 0.8; % [g CaCO3/g beton]

% Korrosionsrate i den forede ledning, angivet for hvert kontrolvolumen
korrosionsrate_vinter = 11.4.*k_korrosion.*((H2S_oxidation_gm2s.*3600)./A); %[mm/aar]
